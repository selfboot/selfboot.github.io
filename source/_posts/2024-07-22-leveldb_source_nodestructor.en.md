...
Of course, even if it doesn't trigger a program crash here, using an already destructed object is [undefined behavior](https://selfboot.cn/2016/09/18/c++_undefined_behaviours/) in C++. Undefined behavior means the program may crash, may run normally, or may produce unpredictable results. The results of such behavior may vary on different systems or at different runtimes, and we must avoid this situation in our development.
