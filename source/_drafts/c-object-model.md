...
**这里参数传递了对象的地址，但是在函数里面是怎么拿到成员变量 b 的地址呢？**我们来看看这个函数的汇编代码，如下：

```shell
(gdb) disassemble /m Basic::setB(double)
Dump of assembler code for function _ZN5Basic4setBEd:
8	    void setB(double value) {
   0x00005555555551d2 <+0>:	push   %rbp
   0x00005555555551d3 <+1>:	mov    %rsp,%rbp
   0x00005555555551d6 <+4>:	mov    %rdi,-0x8(%rbp)
   0x00005555555551da <+8>:	movsd  %xmm0,-0x10(%rbp)

9	        b = value; // 直接访问成员变量b
   0x00005555555551df <+13>:	mov    -0x8(%rbp),%rax
   0x00005555555551e3 <+17>:	movsd  -0x10(%rbp),%xmm0
   0x00005555555551e8 <+22>:	movsd  %xmm0,0x8(%rax)

10	    }
   0x00005555555551ed <+27>:	nop
   0x00005555555551ee <+28>:	pop    %rbp
   0x00005555555551ef <+29>:	ret

End of assembler dump.
```

这里的汇编代码**展示了如何通过 this指针和偏移量访问b**。
